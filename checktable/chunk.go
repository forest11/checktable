...
func (t *TableInfo) GetCrc32CheckSum(where string) (string, error) {
	...
	*/
	...
	var query string
	query = fmt.Sprintf("SELECT COALESCE(LOWER(CONV(BIT_XOR(CAST(CRC32(CONCAT_WS('#',%s)) AS UNSIGNED)), 10, 16)), 0) AS checksum FROM `%s`.`%s` WHERE %s", colsStr, t.dbName, t.tableName, where)
